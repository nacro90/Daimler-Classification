...
class CheckpointManager:
    ...
    def on_batch_completed(self, training_batch_loss, training_batch_accuracy, test_accuracy=None):
        ...
        if test_accuracy:
            self.test_batch_accuracies.append(test_accuracy)
        self._write_current_status()
    ...
    def on_training_completed(self, score="N/A"):
    ...
    def restore_model(self, session, step=0):
        saver = tf.train.Saver()
        meta = tf.train.import_meta_graph(
            self.model_path + "model.ckpt-{}.meta".format(step))
    ...
    def generate_json(self):
        return json.dumps({
            ...
            'test_accuracies': self.test_batch_accuracies
        }, indent=4)
